...
class Blockchain {
    ...
    submitStar(address, message, signature, star) {
        return new Promise(async (resolve, reject) => {
            const expirationTime = parseInt(message.split(':')[1]) + 5 * 60;
            let currentTime = parseInt(new Date().getTime().toString().slice(0, -3));
            if (currentTime < expirationTime) {
                if (bitcoinMessage.verify(message, address, signature)) {
                    resolve(this._addBlock(new Block({
                        owner: address,
                        star,
                    })));
                }
    ...
    getStarsByWalletAddress(address) {
        return new Promise((resolve, reject) => {
            resolve(
                this.chain.map(block => block.getBData()).filter(body => {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            for (let i = 0; i < this.chain.length; i++) {
                ...
                if (block.height > 0 && block.previousBlockHash !== previousHash) {
                    errors.push(`${block.hash} does not link to previous block!`);
                }
